...
// MADCTL BGR bit — value 0x08 is constant across ESP-IDF versions
#ifndef LCD_CMD_BGR_BIT
...
#else
    // ESP-IDF 4.x: disp_off(panel, true) = off, disp_off(panel, false) = on
    // Our function uses on_off=true for ON, so invert when assigning.
...
// on_off=true → display ON (both 4.x and 5.x use this function)
// In 4.x it is assigned to disp_off; the boolean inversion is done at call sites
// via esp_lcd_panel_disp_on_off() which maps correctly through the ops wrapper.
static esp_err_t panel_sh8601_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    esp_lcd_panel_io_handle_t io = sh8601->io;
...
#else
    // 4.x disp_off: on_off == true means "turn off"
    int command = on_off ? LCD_CMD_DISPOFF : LCD_CMD_DISPON;
#endif
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, command, NULL, 0), TAG, "send command failed");
    return ESP_OK;
}
